            ...
            <ul>
                ...
                <li class="food-list food-list-hook" v-for="(item,index) in goods" :key="index">
                    ...
                    <ul>
                        ...
                        @click="showDetail(i)"
                        >
                            <div class="icon" :style="head_bg(i.picture)"></div>
                            <div class="content">
                                <h3 class="name">{{i.name}}</h3>
                                <p class="desc" v-if="i.description">{{i.description}}</p>
                                <div class="extra">
                                    <span class="saled">{{i.month_saled_content}}</span>
                                    <span class="praise">{{i.praise_content}}</span>
                                </div>
                                <img class="product" :src="i.product_label_picture" v-if="i.product_label_picture" />
                                <p class="price">
        ...
        methods:{
            ...
            initScroll(){
                ...
                this.foodScroll = new BScroll(this.$refs.foodScroll,{
                    probeType:3, //probeType为3的时候才会打印滚动的x和y值
                    click:true   //使滚动容器内的所有子元素可以被点击
                })

                //给foodScroll添加监听事件
                this.foodScroll.on("scroll",(pos)=>{
                    this.scrollY = Math.abs(Math.round(pos.y)) //this.scrollY是当前滚动的y值
                    // console.log(this.scrollY)
                })

            },
            calculateHeight(){
                //获取需要对应滚动的商品列表元素(一个元素数组)
...
.goods .menu-wrapper .menu-item:nth-child(4) .text{
    height:34px;
}
